...
if not os.path.exists("./fish_img"):
    ...
    os.makedirs("./fish_img")


req = requests.get('https://animalcrossing.fandom.com/wiki/Fish_(New_Horizons)#Northern%20Hemisphere')
soup = BeautifulSoup(req.text, "html.parser")

fish = soup.find("table", class_="sortable").find_all("tr")

id = 0

file = open("fish.csv", "w")

for f in fish:
    line = ""
    cols = f.find_all("td")

    name = price = loc = shadow = hours1 = hours2 = m_st_1 = m_end_1 = m_st_2 = m_end_2 = ""
    flag_hours = 0
    flag_months = 0
    ...
    if len(cols) > 0:
        ...
        # shadow
        shadow = "shadow: " + cols[4].text.strip()

        # time
        hours = cols[5].small.text
        if hours == "All day":
